function makeLineChart(dataset, xName, yObjs, axisLables) {
    ...
    chartObj.margin = {top: 15, right: 60, bottom: 30, left: 100}; //changed chartobj margin
...
// Get the max of every yFunct
    ...
    //change domain range started from 14000
    chartObj.yScale = d3.scale.linear().range([chartObj.height, 0]).domain([14000, d3.max(chartObj.yFuncts.map(chartObj.max))]);
...
// Build line building functions
    ...
    //change the line style from Cardinal to linear
...
// Render the chart
    chartObj.render = function () {
        ...
        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.yAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", -52).attr("x", -chartObj.height / 2).attr("dy", ".71em").style("text-anchor", "middle").text(chartObj.yAxisLable);
